...
if(isset($_POST['iban'])) {
	if(isset($_SESSION['deposit']['amount'])) {
		...
		if($_SESSION['deposit']['amount'] >= 0.5 && $_SESSION['deposit']['amount'] <= 500) {
			...
			$stmtr = $link->prepare("INSERT INTO deposit_withdraw (username,amount, type, payment, address) VALUES (?,?,'deposit','bank',?)");
			$stmtr->bind_param("sds", $_SESSION['username'], $amount, $_POST['iban']);
			$stmtr->execute();
				...
				<div class="content create">
					...
					<?php
						if(isset($_SESSION['deposit']['amount'])) {
							...
							echo "<label><strong>Send " . number_format($amount, 2) . "$ to our IBAN.</strong> Please type your IBAN so we can check which username belongs to which VisualPay account.</label>";
						}
					?>
					<form action='deposit-bank.php' method='post'>
						<input type='text' placeholder='iban' id='iban' name='iban' required>
						<br>
						<br>
						<label>Our IBAN: <strong>CH20 0079 0042 5999 3006 5</strong></label>
						<br>
						<br>
						<?php
							if(isset($_SESSION['deposit']['amount'])) {
								echo "<label>Make sure, that your IBAN is typed correctly. We won't do any refund!</label>";
							}
						?>
						<br>
						<br>
						<label><strong>IMPORTANT: </strong>Set the transaction text to your VisualPay username.</label>
