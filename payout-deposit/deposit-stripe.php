...
if(isset($_POST['email'])) {
	if(isset($_SESSION['deposit']['amount'])) {
		...
		if($_SESSION['deposit']['amount'] >= 0.5 && $_SESSION['deposit']['amount'] <= 500) {
			...
			$stmtr = $link->prepare("INSERT INTO deposit_withdraw (username,amount, type, payment, address) VALUES (?,?,'deposit','stripe',?)");
			$stmtr->bind_param("sds", $_SESSION['username'], $amount, $_POST['email']);
			$stmtr->execute();
			unset($_SESSION['deposit']);
			
			//TODO: Error Message
			$_SESSION['message']['success'] = 'We received the request. We will send you the Payment Link very soon..';
		}
		...
		<div class="content">
			...
			<h2>Deposit</h2>
				...
				<div class="content create">
                    <label>Please type your Email so we can send the Payment Link.</label>

					<form action='deposit-stripe.php' method='post'>
						<input type='text' placeholder='Your Email Adress' id='email' name='email' required>
						<br>
						<br>
						...
						<br>
						<input type='submit' name='submit' placeholder='Deposit' value='Send me the Link'>
					</form>
