...
if(isset($_POST['cryptoaddress'])) {
	if(isset($_SESSION['deposit']['amount'])) {
		...
		if($_SESSION['deposit']['amount'] >= 0.5 && $_SESSION['deposit']['amount'] <= 500) {
			...
			$stmtr = $link->prepare("INSERT INTO deposit_withdraw (username,amount, type, payment, address) VALUES (?,?,'deposit','crypto',?)");
			$stmtr->bind_param("sds", $_SESSION['username'], $amount, $_POST['cryptoaddress']);
			$stmtr->execute();
		...
		<div class="content">
			...
			<h2>Deposit</h2>
				...
				<div class="content create">
					...
					<h5>We accept only Litecoin! We won't refund, if you type the wrong address.</h5>
				
					<?php
						if(isset($_SESSION['deposit']['amount'])) {
							$amount = $_SESSION['deposit']['amount'];
							
							echo "<label>You will receive " . number_format($amount, 2) . "$ on VisualPay after depositing</label>";
						}
					?>
					<form action='deposit-crypto.php' method='post'>
						<input type='text' placeholder='Your LTC Address' id='cryptoaddress' name='cryptoaddress' required>
						<br>
						<br>
						<label>Our LTC address: <strong>MFNB9h5LPx6NAgQmbXLc5BGhzU31FnUCnM</strong></label>
						<br>
						<br>
						<?php
							if(isset($_SESSION['deposit']['amount'])) {
								echo "<label>Make sure, that the LTC address is typed correctly. We won't do any refund!</label>";
							}
						?>
